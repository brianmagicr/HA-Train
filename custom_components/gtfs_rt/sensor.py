...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    data = PublicTransportData(config.get(CONF_TRIP_UPDATE_URL), config.get(CONF_VEHICLE_POSITION_URL), config.get(CONF_X_API_KEY))
    sensors = []
...
class PublicTransportSensor(Entity):
    ...
    def state(self):
        ...
        return due_in_minutes(next_trains[0].arrival_time) if len(next_trains) > 0 else '-'
